...
const users = [
  {
    ...
    name: "Joy Brar",
    username: "@joybrar",
    avatar: "https://avatars.githubusercontent.com/u/194884352?v=4",
  },
  {
    id: 2,
    name: "Alice Smith",
    username: "@alice",
    avatar:
      "https://images.unsplash.com/photo-1564564295391-7f24f26f568b?q=80&w=2076&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  },
  {
    id: 3,
    name: "Bob Johnson",
...
export function ProfileSelectorControlled() {
  ...
  const handleSelectUser = (user: (typeof users)[number]) => {
    setOtherUsers([
      selectedUser,
      ...otherUsers.filter((u) => u.id !== user.id),
    ]);
      ...
      <CollapsibleContent>
        {otherUsers.map((user) => (
          <CollapsibleCard
            key={user.id}
            variant="outline"
            className="flex gap-2 items-center py-1 hover:bg-neutral-800"
